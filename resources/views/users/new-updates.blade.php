...
@section('content')
    <div class="container">
        <div class="page-breadcrumb">
            ...
            <span class="item breadcrumb_last" aria-current="page">Mới cập nhật</span>
        </div>

        <section class="mb-3">
            <div class="group-title">
                <div class="only-title">
                    <h1 class="m-title title">Truyện tranh mới cập nhật</h1>
                </div>
            </div>
                            ...
                            <a title="{{$item->name}}" href="{{route('detail', ['slug' => $item->slug])}}">
                                <span class="img-poster">
                                    <img class="lzl" data-src="{{$item->thumbnail}}" rel="nofollow"
                                        data-original="{{$item->thumbnail}}" alt="{{$item->name}}" width="100%" height="100%">
                                </span>
                            </a>
                        </div>
                        <div class="p-content flex-grow-1">
                            ...
                            <ul class="list-chaps">
                                ...
                                @foreach ($item->chapters->take(-1)->reverse() as $chap)
                                    <li class="chapter">
                                        <a data-id="{{$chap->id}}"
                                            href="{{route('showRead', ['slug' => $item->slug, 'chapter' => $chap->slug])}}"
                                            title="Chapter #{{$chap->name}}">
