...
@section('main_content')
    <div class="wrapper">
            ...
            <div class="reading-page">
                <header class="reading-header" id="readingHeader">
                    <div class="r-left">
                        ...
                        <h1 class="manga-name">
                            Đọc <a href="{{route('detail', ['slug' => $comic->slug])}}">{{$comic->name}}</a> - Chapter
                    ...
                    <div class="r-right">
                        ...
                        <a href="#" class="show-setting d-flex" data-bs-toggle="offcanvas"
                            data-bs-target="#offcanvasSetting">
                            <i class="icon-setting-2"></i>
                        </a>

                    </div>
                    <div class="r-navigation navigation navi-chap position-relative">
                        <button type="button" class="navi prev @isset($comic->prevChap)  @else disabled @endisset"
                            onclick="handlePrevChapter()" title="Chapter Trước">
                            <i class="icon-arrow-left"></i>
                        </button>

                        <div id="dd-chapters" class="dropdown">
                            <a href="" class="dropdown-toggle" id="dropdownChaps" data-bs-toggle="dropdown"
                                aria-expanded="false">
                                <span>
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownChaps">
                                ...
                                <div class="list-chap">
                                    @foreach ($comic->chapters->reverse() as $chapter)
                                        <span
                                            class="l-chapter @if (request()->route('chapter') == $chapter->slug) selected @endif">
                                            <a class="dropdown-item" data-value="Chapter #{{$chapter->name}}"
                                                href="{{route('showRead', ['slug' => $comic->slug, 'chapter' => $chapter->slug])}}"
                            ...
                            <div class="dropdown-menu">
                                @foreach ($servers as $server)
                                    <span class="{{$server->id == $chapterSelected->id ? 'active' : ''}}">
                                        <span class="dropdown-item" data-value="{{$server->server}}"
                                            href="">{{$server->server}}</span>
                                    </span>
                                @endforeach
                            </div>
                        </div>

                        <button type="button" class="navi next @isset($comic->nextChap)  @else disabled @endisset"
                            onclick="handleNextChapter()" title="Chapter Sau">
        ...
        <div href="javascript:void(0)" id="back-to-top"
            style="display: flex; cursor: pointer; opacity: 1; transition: opacity 0.3s;">
            ...
            <div class="list-settings">
                ...
                <div class="reading-mode disabled">
                    <div id="dd-mode" class="dropdown">
                        <a href="" class="dropdown-toggle" id="dropdownMode" data-bs-toggle="dropdown"
                            aria-expanded="false">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMode">
                            <div class="list-mode">
                                ...
                                <span><a class="dropdown-item" data-mode="horizon-single" data-value="Ngang 1 trang"
                                        href="#">Ngang 1 trang</a></span>
                                <span><a class="dropdown-item" data-mode="horizon-double" data-value="Ngang 2 trang"
                                        href="#">Ngang 2 trang</a></span>
                            </div>
                        </div>
                    </div>
                ...
                <div id="lightmode" class="dropdown">
                    <a href="" class="dropdown-toggle" id="dropdownLightmode" data-bs-toggle="dropdown"
                        aria-expanded="false">
    ...
    <div class="modal fade" id="ReportModal" tabindex="-1" aria-labelledby="ReportModalLabel" style="display: none;"
        aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="report-form">
                        ...
                        <button type="button" class="btn" data-bs-toggle="modal" data-bs-target="#otp-modal">Báo
                            cáo</button>
                    </form>
                </div>
            </div>
    ...
    <script>
        async function postReport(mangaId, reasons, chapterId) {
            const response = await $.ajax({
                ...
                xhrFields: { withCredentials: true },
                url: chapterId
                    //TODO: Add API Report chapter here
                    ? `#/mangas/${mangaId}/${chapterId}/report`
                    : `#/mangas/${mangaId}/report`,
                contentType: 'application/json',
